...
- **Proceso de la API.** La imagen usa `python:3.12-slim-bookworm`, instala dependencias como root y arranca gunicorn con el usuario `app`. El código de ejecución es el de la imagen (`/app`). El editor del Codespace es un contenedor aparte; el volumen `.:/workspace` solo monta el repo dentro de `api`.